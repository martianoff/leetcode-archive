      ...
      def parse(): ASTExp = {
        ...
        while (operator.nonEmpty) {
          ...
          (left, right, operator) match {
            // the first operation
            case (Some(l), Some(r), Some(Operator(op))) =>
              astNode = ASTNode(tokenToASTExp(l), tokenToASTExp(r), op)
              left = None
            // all other operations
            case (None, Some(r), Some(Operator(op))) =>
              astNode = ASTNode(astNode, tokenToASTExp(r), op)
            case _ => throw new Exception("invalid")
          }
          operator = eat()
        }
